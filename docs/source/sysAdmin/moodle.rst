...
.. note::
    Logstore xAPI has an option called ``Send statements by scheduled task?``. This will force Moodle to send the statements to Learning Locker in the background, via a cron task to avoid blocking page responses.
